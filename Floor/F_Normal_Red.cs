...
public class F_Normal_Red : F_Normal
{
    float fat_threshold;
    ...
    public override IEnumerator MakeHuman()
    {
        while (true)
        {
            ...
            Gamemanager.Instance.objectpool.GetRed(fat_threshold).GetComponent<Human>().Set(this);
        }
    }
    public override IEnumerator MakeRush()
    {
        ...
        while (Gamemanager.Instance.buildgame.timer_arr[FID.RED].rush_on)
        {
            yield return new WaitForSeconds(Random.Range(min_respawntime, max_respawntime) * Gamemanager.Instance.buildgame.night_regen);
