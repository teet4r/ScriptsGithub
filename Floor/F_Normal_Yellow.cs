...
public class F_Normal_Yellow : F_Normal
{
    ...
    public override IEnumerator MakeHuman()
    {
        while (true)
        {
            ...
            Gamemanager.Instance.objectpool.GetYellow(fat_threshold).GetComponent<Human>().Set(this);
        }
    }
    public override IEnumerator MakeRush()
    {
        ...
        while (Gamemanager.Instance.buildgame.timer_arr[FID.YELLOW].rush_on)
        {
            yield return new WaitForSeconds(Random.Range(min_respawntime, max_respawntime) * Gamemanager.Instance.buildgame.night_regen);
